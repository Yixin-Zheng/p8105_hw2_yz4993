...
```{r 2.1 data import and cleaning}
trash_wheel = 
  read_excel(
  ...
  select(1:14) |>     # select only the first 14 columns (bc only 14 variables)
  filter(!is.na(dumpster)) |>        # omit rows without dumpster-specific data
  mutate(
    sports_balls = as.integer(round(sports_balls)))  # Round & change to integer
